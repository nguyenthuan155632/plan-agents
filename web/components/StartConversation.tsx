...
export default function StartConversation({ onSessionCreated, hasActiveSession = false }: StartConversationProps) {
  ...
  return (
    <div className="bg-white rounded-none neo-border neo-shadow">
      ...
      <div className={`overflow-hidden transition-all duration-300 ${isCollapsed ? 'max-h-0' : 'max-h-[600px]'}`}>
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 p-4 sm:p-6 pt-0">
          ...
          {error && (
            <div className="p-3 sm:p-4 bg-red-300 neo-border-thin text-black text-xs sm:text-sm font-bold font-mono">
              ⚠️ {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={isStarting}
            className="w-full px-4 sm:px-6 py-3 sm:py-4 bg-green-400 hover:bg-green-500 active:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-black font-black rounded-none neo-border neo-shadow-hover uppercase tracking-wide text-base sm:text-lg flex items-center justify-center touch-manipulation font-mono"
          >
            {isStarting ? (
              <>
