    ...
    <div className="bg-white dark:bg-gray-900 rounded-none neo-border neo-shadow p-6">
      ...
      {sessions.length === 0 ? (
        <div className="text-gray-600 dark:text-gray-400 text-center py-8">
          ...
          <p className="text-sm mt-2">Start one using the CLI</p>
        </div>
      ) : (
          ...
          {sessions.map((session) => (
            <button
              ...
              className={`w-full text-left p-4 rounded-none neo-border-thin transition-all ${selectedSession === session.id
                ? 'bg-yellow-300 dark:bg-yellow-400 neo-shadow-lg'
                : 'bg-gray-100 dark:bg-gray-800 neo-shadow hover:neo-shadow-lg hover:-translate-x-0.5 hover:-translate-y-0.5'
                }`}
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  <h3 className="font-black text-black dark:text-white line-clamp-1 break-words text-base">
                ...
                <div className={`w-3 h-3 rounded-none border-2 border-black dark:border-white ${getStatusColor(session.status)} mt-1 ml-2 flex-shrink-0`}></div>
              </div>

              <div className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400 font-bold uppercase">
