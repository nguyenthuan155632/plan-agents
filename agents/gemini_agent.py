...
class GeminiAgent(BaseAgent):
    ...
    def __init__(self, role: Role, db, config: dict = None):
        ...
        # Define personality based on role (same as GLM agent)
    ...
    def _get_system_prompt(self) -> str:
        """Get system prompt based on agent role - SAME as GLM agent."""
...
Previous message: {previous_message.content[:200]}...
...
Respond naturally and briefly (150-200 words)."""
            ...
            logger.info(f"{self.role.value} (Gemini) generated {len(generated_text)} chars using {self.model_name}")
            ...
            if self.detected_language == 'vietnamese':
                return f"Xin lỗi, tôi gặp sự cố với Gemini API. Hãy thử lại hoặc Agent A sẽ tiếp tục."
            else:
                return f"I apologize, I'm having trouble with Gemini API. Please try again or Agent A will continue."
